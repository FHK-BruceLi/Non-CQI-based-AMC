...
simParameters.SNRIn = -8:1:29;%[-5 0 5]; % SNR range (dB)
% Array to store the maximum throughput for all SNR points
maxThroughput = zeros(length(MCS_idx_list),length(simParameters.SNRIn)); 
% Array to store the simulation throughput for all SNR points
simThroughput = zeros(length(MCS_idx_list),length(simParameters.SNRIn));
...
for MCS_sim_run = 1:length(MCS_idx_list)
        [ModulationType, CodingRate]=MCSidx2MCS(MCS_table,MCS_idx_list(MCS_sim_run));
        ...
        fprintf('\nSimulating MCS Table %d, Index %d with %s Modulation Type and %d Code Rate\n',...
                MCS_table, MCS_idx_list(MCS_sim_run),ModulationType,CodingRate); 
        %%
        ...
        % set the 'displaySimulationInformation' variable above to false
            ...
            SNRdB = snrIn(snrIdx);
            fprintf('\nSimulating %s-based transmission scheme (%dx%d) and SCS=%dkHz with %s channel at %gdB SNR for %d 10ms frame(s), Index %d\n',pusch.TxScheme,nTxAnts,nRxAnts,ue.SubcarrierSpacing,channelType,SNRdB,ue.NFrames, MCS_idx_list(MCS_sim_run));
            ...
            while ue.NSymbol < NSymbols
                ...
                simThroughput(MCS_sim_run,snrIdx) = simThroughput(MCS_sim_run,snrIdx) + (~harqProcesses(harqProcIdx).blkerr * TBS);
                maxThroughput(MCS_sim_run,snrIdx) = maxThroughput(MCS_sim_run,snrIdx) + TBS;
            ...
            fprintf([['\nThroughput(Mbps) for ' num2str(ue.NFrames) ' frame(s) '],'= %.4f\n'], 1e-6*simThroughput(MCS_sim_run,snrIdx)/(ue.NFrames*10e-3));
            fprintf(['Throughput(%%) for ' num2str(ue.NFrames) ' frame(s) = %.4f\n'],simThroughput(MCS_sim_run,snrIdx)*100/maxThroughput(MCS_sim_run,snrIdx));
...
cd Sim_results\LUT_sim\
save simThroughput_1dBstep_CDL_A_noHARQ.mat simThroughput
save maxThroughput_1dBstep_CDL_A_noHARQ.mat maxThroughput
save simParameters_1dBstep_CDL_A_noHARQ.mat simParameters
cd ..\..
...
hold on
for MCS_sim_run = 1:length(MCS_idx_list)
    plot(snrIn,(100 - simThroughput(MCS_sim_run,:)*100./maxThroughput(MCS_sim_run,:)),'o-.','LineWidth', 1.5)
end
line([min(snrIn), max(snrIn)],[10, 10],'Color','red','LineStyle','-','LineWidth',1)
hold off
xlabel('SNR (dB)'); ylabel('BLER (%)'); grid on;
title(sprintf('ULSCH SNR-to-MCS Mapping, MCS table-2, HAQR disabled, 1T1R, CDL-A'));
